...
object ClosedIngest {
   ...
   * @param digest        function that utilize the result of the mosaic
  ...
  def apply[T: IngestKey: ClassTag, K: SpatialComponent: ClassTag]
    ...
    (digest: (LayoutLevel, RasterRDD[K]) => Unit)
    (implicit tiler: Tiler[T, K]): Unit =
  {
    ...
    val rasterRdd = tiler(reprojectedTiles, rasterMetaData).persist(StorageLevel.MEMORY_ONLY)    
    ...
    digest(layoutLevel, rasterRdd)                  // let there be side effects
    rasterRdd.unpersist(blocking = false)           // cleanup now
  }
}
