...
trait GeometryFormats {
  ...
  private implicit object PointFormat extends RootJsonFormat[Point] {
  ...
  private implicit object LineFormat extends RootJsonFormat[Line] {
  ...
  private implicit object PolygonFormat extends RootJsonFormat[Polygon] {
  ...
  private implicit object MultiPointFormat extends RootJsonFormat[MultiPoint] {
  ...
  private implicit object MultiLineFormat extends RootJsonFormat[MultiLine] {
  ...
  private implicit object MultiPolygonFormat extends RootJsonFormat[MultiPolygon] {
  ...
  implicit object GeometryCollectionFormat extends RootJsonFormat[Seq[Geometry]] {
    def write(geoms: Seq[Geometry]) = JsObject(
      "type" -> JsString("GeometryCollection"),
      "geometries" -> JsArray(geoms.map {
        o => o.toJson
      }.toList)
    )
    ...
    def read(value: JsValue) = value.asJsObject.getFields("geometries") match {
      case Seq(JsArray(geoms)) =>
        //Vector is the "safest" collection we can give.
        //We have to be careful here because of the weird contra-variance going on with GeoReader
        (for (geom <- geoms) yield geom.convertTo[Geometry]).toVector
    }
  }
