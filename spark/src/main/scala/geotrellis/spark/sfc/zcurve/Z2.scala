...
class Z2(val z: Long) extends AnyVal {
  ...
  override def toString = f"$z (${dim(0)}, ${dim(1)})"
}

object Z2 {  
  final val MAX_BITS = 31
  final val MAX_MASK = 0x7fffffff // ignore the sign bit, maybe a way to avoid this
  final val MAX_DIM = 2
  ...
  def echo(name: String, x: Z2, note: String = "") = println(f"$name%8s: ${x}      $note")
  
  def zdivide(p: Z2, rmin: Z2, rmax: Z2): (Z2, Z2) = {
    val (litmax,bigmin) = geotrellis.spark.sfc.zcurve.zdivide(load, MAX_DIM)(p.z, rmin.z, rmax.z)
  ...
  def zranges(min: Z2, max: Z2): Seq[(Long, Long)] = {
    ...
    def _zranges(prefix: Long, offset: Int, quad: Long): Unit = {      
      ...
      // TODO : check for case of half-overlap here, it really only saves one recursive call
    }
